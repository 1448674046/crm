...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#saveBundActivityBtn").click(function () {
			...
			$.each(checkIds, function () {
				ids+="activityId="+this.value+"&";	// this.value 各自选中框的值
			});
			ids+="clueId="+clueId;
			...
			$.ajax({
				...
				type :'post',
				dataType : 'json',
				success : function (data) {	// data == returnObject {code, message, retData == activityList{a1, a2, a3 ... } }
					if (data.code == "1") {
		 ...
		 * 为 "解除关联" 按钮绑定单击事件
		 */
		$("#relationTBody").on("click", "a", function () {	// "a"：为relationTBody容器中所有超链接绑定单击事件
			// alert("解除关联");
			// 删除时根据 clueId 和 activityId 来删除
			// 收集数据
			var clueId = "${clue.id}";
				...
				$.ajax({
					...
					success : function (data) {		// data = returnObject { code, message, retData }
						if (data.code == "1") {
							...
							html += '<font color="gray">线索</font> <font color="gray">-</font> <b>${clue.fullName}${clue.appellation}-${clue.company}</b> <small style="color: gray;">' + (data.retData.editFlag == "0"? data.retData.createTime : data.retData.editTime ) + ' 由 ' + "${sessionScope.sessionUser.name}" + ' 创建</small>';
							html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
							html += '<a class="myHref" href="javascript:void(0);" onclick="editClueRemarkById(\'' + data.retData.id + '\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #E6E6E6;"></span></a>';
							html += '&nbsp;&nbsp;&nbsp;&nbsp;';
							html += '<a class="myHref" href="javascript:void(0);" onclick="deleteClueRemarkById(\''+ data.retData.id +'\')"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #E6E6E6;"></span></a>';
							html += '</div>';
							html += '</div>';
							html += '</div>';

							// 将其拼接到 remarkDiv 图层的上方
	...
	function showClueRemarkList() {
		$.ajax({
			url : 'workbench/clueRemark/getClueRemarkListByClueId.do',
			data : {
				clueId : '${clue.id}'
			},
			type : 'get',
			dataType : 'json',
			success : function (data) {	// data == returnObject { code, message, retData == clueRemarkList {cr1,cr2,cr3, ...} }
				var html = "";
				$.each(data.retData, function (i, n) {
					...
					html += '<font color="gray">线索</font> <font color="gray">-</font> <b>${clue.fullName}${clue.appellation}-${clue.company}</b> <small style="color: gray;" id="etb'+n.id+'">' + (n.editFlag==0? n.createTime: n.editTime) + ' 由 ' + (n.editFlag==0? n.createBy: n.editBy) + ' ' + (n.editFlag==0? '创建': '更新') +'</small>';
					html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
					html += '<a class="myHref" href="javascript:void(0);" onclick="editClueRemarkById(\'' + n.id + '\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #FF0000;"></span></a>';
					html += '&nbsp;&nbsp;&nbsp;&nbsp;';
					html += '<a class="myHref" href="javascript:void(0);" onclick="deleteClueRemarkById(\'' + n.id + '\')"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #FF0000;"></span></a>';
					html += '</div>';
					html += '</div>';
					html += '</div>';
				});
	...
	function deleteClueRemarkById(id) {
		// alert(id);
		if (window.confirm("确定删除该线索下的这条备注吗？")) {
			$.ajax({
	...
	// 根据 id 编辑线索备注
	function editClueRemarkById(id) {
		// alert(id);
		// 1.单击修改图标打开模态框，打开模态框之前经过后台根据 id 查询线索备注
		$.ajax({
			url : 'workbench/clueRemark/editClueRemarkById.do',
			data : {
				id : id
			},
			type : 'get',
			dataType : 'json',
			success : function (data) {		// data == returnObject{ code, message, retData == clueRemark }
				if (data.code == "1") {
					$("#clueRemarkId").val(id);
	...
	// 根据 id 保存编辑线索备注
    function saveEditClueRemark() {
		if (window.confirm("是否确认更新该线索下的此条备注？")) {
			var id =  $("#clueRemarkId").val();
			var noteContent = $.trim($("#noteContent").val());
			// alert(clueRemarkId + " : " + noteContent);
			$.ajax({
				...
				type : 'post',
				dataType : 'json',
				success : function (data) {		// data == returnObject{ code, message, retData == clueRemark{属性} }
					if (data.code == "1") {
...
<body>
        ...
        <div class="modal-dialog" role="document" style="width: 90%;">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="ModalLabel">修改线索</h4>
                </div>
                <div class="modal-body">
                    <form class="form-horizontal" role="form">
	...
	<div>
		<div style="position: relative; top: 60px; left: 40px;">
			...
			<div>
				<a href="javascript:void(0);" id="bundActivityBtn" data-toggle="modal" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
