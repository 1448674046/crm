...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#searchBtn").click(function () {
			queryTranForPageByCondition(1, $("#tranPage").bs_pagination('getOption', 'rowsPerPage'));
			// alert("search-tranList");
		});
	...
	function queryTranForPageByCondition(pageNo, pageSize) {
		...
		$.ajax({
			url : 'workbench/transaction/queryTranListForPageByCondition.do',
			data : {
					  'pageNo' : pageNo,
					'pageSize' : pageSize,
				          'id' : id,
			...
			type : 'get',
			dataType : 'json',
			success : function (data) {	// data == map<String, Object> { tranList { tran1, tran2, ... }, total }
				var html = "";
				$.each(data.tranList, function (i, n) {
					html += '<tr class="active">';
					html += '<td><input type="checkbox" value="' + n.id + '"/></td>';
					html += '<td><a style="text-decoration: none; cursor: pointer;" onclick="window.location.href=\'workbench/transaction/editTran.do?id=' + n.id + '\';">' + n.name + '</a></td>';
					html += '<td>' + n.customerId + '</td>';
					html += '<td>' + n.stage + '</td>';
					html += '<td>' + (n.type == null? '未知' : n.type) + '</td>';
					html += '<td>' + n.owner + '</td>';
					html += '<td>' + (n.source == null? '未知' : n.source) + '</td>';
				...
				$("#tranPage").bs_pagination({
					...
					onChangePage : function(event, data){
						queryTranForPageByCondition(data.currentPage , data.rowsPerPage);
					}
				});
			}
		});
		...
		<div style="width: 100%; position: absolute;top: 5px; left: 10px;">
				  ...
				  <div class="form-group">
				    <div class="input-group">
				      <div class="input-group-addon">所有者</div>
						<input id="search-tranId" type="hidden">
				    ...
				    <div class="input-group">
					  ...
					  <select id="search-stage" class="form-control">
						...
						<c:forEach items="${stageList}" var="stageList" >
							<option value="${stageList.value}">${stageList.text}</option>
						</c:forEach>
					  	<%--
					  	<option></option>
					  	<option>资质审查</option>
			...
			<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;top: 10px;">
				<div class="btn-group" style="position: relative; top: 18%;">
				  ...
				  <button type="button" class="btn btn-default" onclick="window.location.href='workbench/transaction/editTran.do';"><span class="glyphicon glyphicon-pencil"></span> 修改</button>
